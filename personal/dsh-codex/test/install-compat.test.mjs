...
test('installed-build compatibility is version checked, backed up once, and repeatable', async () => {
  ...
  try {
    ...
    const paths = {};
    for (const name of ['session', 'commands', 'client-ui-commands']) {
      const dir = join(root, 'node_modules', '@deepseek-ai', 'dsh-' + name);
      await mkdir(join(dir, 'lib'), { recursive: true });
      await writeFile(join(dir, 'package.json'), JSON.stringify({ version, main: 'lib/index.js', exports: {'.':'./lib/index.js','./client':'./lib/index.js'} }));
    ...
    await writeFile(paths['client-ui-commands'], '// old command menu\n');
