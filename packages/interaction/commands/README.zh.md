...
作用域内的 Harness 预设可以调用 `ctx.commands.useNativePalette()`，将继承的菜单替换为原生命令。全局 DSH 命令仍通过 `/dsh <command>` 调用；单独输入 `/dsh` 可列出它们。返回的清理函数恢复继承的菜单。命令处理器继续执行各自的输入与附件检查。每个会话的原生技能目录必须注册到真实 Agent 的作用域中，不能注册到多个会话共享的常驻预设作用域。
